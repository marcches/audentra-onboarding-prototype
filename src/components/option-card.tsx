...
export function OptionCard({
  ...
  return (
    <label
      htmlFor={id}
      className={cn(
        "group flex cursor-pointer items-start gap-2 rounded-[var(--radius-field)] border border-ink-200 bg-panel px-2 py-2 compact:min-h-[var(--tap-target)]",
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col">
        ...
        {consequence ? (
          <span className="text-meta leading-4 text-ink-500">{consequence}</span>
        ) : null}
      </span>
    </label>
  );
